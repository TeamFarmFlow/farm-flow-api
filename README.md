...
실제 새송이 버섯 농장 운영 흐름을 소프트웨어로 옮기기 위해 개발 중인 농장 관리 백엔드 MVP 프로젝트입니다.  
부모님이 운영하는 농장에서 바로 사용할 수 있는 시스템을 목표로, 농장 멤버십과 권한 관리부터 생육동 운영, 재배 주기 추적, 환경 기록, 출퇴근 관리까지 하나의 도메인으로 통합하고 있습니다.

## 프로젝트 소개

FarmFlow API는 실제 새송이 버섯 농장의 운영 프로세스를 기반으로, 데이터 흐름과 권한 구조를 중심에 두고 설계한 백엔드 MVP 프로젝트입니다.

단순히 기능을 나열하는 CRUD API를 만드는 것보다, 농장 운영에서 반복적으로 발생하는 사용자 권한, 재배 사이클, 환경 데이터 기록을 어떻게 구조적으로 모델링할 것인지에 더 집중했습니다.
...
또한 권한은 단순한 전역 `ADMIN`, `USER` 방식이 아니라 농장 안에서의 역할로 분리했습니다.

- 어떤 농장에서는 `OWNER`
- 다른 농장에서는 `WORKER`

처럼 동작할 수 있도록 모델링했습니다.  
시스템 기본 역할뿐 아니라 커스텀 역할까지 확장할 수 있도록 `Role`, `RolePermission` 구조를 도입했습니다.

### 2. 도메인 분리

도메인을 단순 테이블 나열이 아니라 책임 단위로 나누는 것을 중요하게 봤습니다.
...
각 도메인이 독립적인 책임을 가지도록 분리해, 유지보수성과 향후 기능 확장 가능성을 고려했습니다.
...
- 백엔드 MVP API 구현 완료
- 인증 흐름 연동 및 핵심 도메인 로직 구현 완료
- 로컬 환경 기준 Swagger 기반 API 확인 가능
- 프론트엔드 및 배포 환경은 향후 확장 예정

## 구현 범위

- 프로젝트의 주요 기여 범위는 농장 도메인 모델링, 권한 구조 설계, 재배 사이클 및 운영 API 구현입니다.
- 인증/토큰 처리 일부는 기존 구현 또는 학습용 레퍼런스를 바탕으로 프로젝트에 맞게 연동했습니다.
- 따라서 이 프로젝트에서는 인증 자체보다 농장 운영 흐름을 어떻게 API와 데이터 모델로 구조화했는지에 더 초점을 맞췄습니다.
