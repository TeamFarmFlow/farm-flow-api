...
@Table(
    ...
    indexes = {
      ...
      @Index(name = "idx_invite_code_hash", columnList = "invite_code_hash")
    },
    uniqueConstraints = {
      @UniqueConstraint(
          name = "uk_invite_code_hash",
...
public class FarmInvitation extends BaseTimeEntity {
  ...
  @Column(name = "invite_code_hash", nullable = false, length = 128)
  ...
  public void update(FarmInvitationStatus status, User inviteeUser) {
    this.status = status;
